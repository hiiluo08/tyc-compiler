...
export function AstPanel({ result }: AstPanelProps) {
  ...
  const astText = useMemo(() => {
    if (!result) return "Run or request AST to see tree data.";
    if (result.astText) return result.astText;
    if (result.astJson) return JSON.stringify(result.astJson, null, 2);
  ...
  return (
    <div className="panel-content">
      <div className="ast-mode-switch" role="tablist" aria-label="AST view mode">
        <button
          className={viewMode === "tree" ? "ast-mode-active" : ""}
        ...
        <button
          className={viewMode === "text" ? "ast-mode-active" : ""}
