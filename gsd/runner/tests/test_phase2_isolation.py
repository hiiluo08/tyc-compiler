...
BASELINE = pathlib.Path("gsd/.planning/phases/2/isolation-baseline.json")
...
def test_phase2_isolation_delta() -> None:
    assert BASELINE.exists(), "missing phase 2 baseline"
...
def test_phase2_no_new_root_runtime_or_build_artifacts() -> None:
    baseline = json.loads(BASELINE.read_text(encoding="utf-8"))
    baseline_status = set(baseline.get("status_lines", []))
    now_status = set(subprocess.check_output(["git", "status", "--porcelain"], text=True).splitlines())
